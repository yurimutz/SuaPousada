...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 4. Criar 30 Quartos (Maior ocorrência dos baratos)
        List<QuartoResponseDTO> quartos = new ArrayList<>();
        int quartoNumero = 101;
        for (int i = 0; i < 30; i++) {
            Long tipoQuartoId;
            if (i < 18) tipoQuartoId = tqBaixo.id();        // 18 Standard
            else if (i < 26) tipoQuartoId = tqMedio.id();   // 8 Master
            else tipoQuartoId = tqAlto.id();                // 4 Presidential

            QuartoResponseDTO quarto = quartoService.create(new QuartoCreateRequestDTO(
                    quartoNumero++,
                    i < 15 ? 1 : 2, // 1º e 2º Andar
                    tipoQuartoId
            ));
            quartos.add(quarto);
        }

        // 5. Criar Reservas para Clientes (1 a 3 por cliente)
        for (ClienteResponseDTO cliente : clientes) {
            int qtdReservas = random.nextInt(3) + 1; // 1 a 3 reservas
